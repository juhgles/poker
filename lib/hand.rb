class Hand
  ...
  def winning_hand_type
    find_strongest_combo
    @winning_hand_type
  end
  ...
  def straight_flush
    (flush == @current_hand && straight == @current_hand) ? @current_hand : []
  end

  def four_of_a_kind
    four_kind = []
    @current_hand.each do |card|
      four_kind << card if @current_hand.count{|other_card| other_card.value == card.value} == 4
    end
    four_kind
  end
  ...
  def straight
    ...
    card_values.each_with_index do |el,index|
      unless index == card_values.length-1
  ...
  def three_of_a_kind
    trio = []
    @current_hand.each do |card|
      trio << card if @current_hand.count{|other_card| other_card.value ==card.value} == 3
    end
    trio
  end
  ...
  def two_pair
    pair = []
    @current_hand.each do |card|
      pair << card if @current_hand.count{|other_card| other_card.value ==card.value} == 2
    end
    pair.length == 4 ? pair : []
  end
